...
function IncidentDrawer({ incident, onClose, onUpdated }) {
  ...
  return <div className="drawer-scrim" onClick={onClose}><aside className="incident-drawer" onClick={(e) => e.stopPropagation()}><div className="drawer-head"><div><div className="eyebrow">Incident investigation</div><h2>{titleCase(incident.incident_type)}</h2></div><button className="icon-btn" onClick={onClose}><X size={18} /></button></div><div className="drawer-severity"><span className={`incident-marker marker-${String(incident.severity || '').toLowerCase()}`}><ShieldAlert size={18} /></span><div><strong>{titleCase(incident.severity)} severity signal</strong><span>{relativeTime(incident.timestamp)} · {titleCase(displayStatus(incident.status))}</span></div></div><div className="detail-grid"><Detail label="Incident ID" value={incident.id} /><Detail label="Bus ID" value={incident.bus_id} /><Detail label="Detection time" value={shortTime(incident.timestamp)} /><Detail label="Location" value={coord(incident.location)} /><Detail label="AI confidence" value={confidence(incident.confidence)} /><Detail label="Vehicle number" value={incident.vehicle_number || 'Not identified'} /></div><EvidencePreview evidencePath={incident.evidence_url} /><label className="evidence-upload"><FileImage size={15} /><span>{evidence ? evidence.name : 'Add photo evidence'}</span><input type="file" accept="image/*" onChange={(event) => setEvidence(event.target.files?.[0] || null)} /></label><div className="drawer-actions"><span>Update status</span><div className="status-choice-row">{['PENDING', 'ACKNOWLEDGED', 'IN_PROGRESS'].map((value) => <button key={value} className={selectedStatus === value ? 'selected' : ''} disabled={busy} onClick={() => setSelectedStatus(value)}>{titleCase(value)}</button>)}</div><button className="drawer-save" disabled={busy} onClick={save}>{busy ? 'Saving…' : <><UploadCloud size={15} /> Save update</>}</button>{error && <small className="drawer-error">{error}</small>}</div></aside></div>
}
...
function EvidencePreview({ evidencePath }) {
  ...
  return <div className="evidence-preview"><span>Photo evidence</span><a href={url} target="_blank" rel="noreferrer"><img src={url} alt="Incident evidence" /></a></div>
}
