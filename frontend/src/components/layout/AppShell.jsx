...
import { Activity, Bell, BusFront, ChevronRight, CircleHelp, LayoutDashboard, Map, Menu, Moon, Radio, Settings, ShieldAlert, Sun, TrafficCone, X } from 'lucide-react'
...
export default function AppShell({ children, apiStatus, realtimeStatus, notifications = [], dismissNotification, theme = 'dark', onToggleTheme }) {
  const [open, setOpen] = useState(false)
  const location = useLocation()
  const pageLabel = nav.find((item) => item.path === location.pathname)?.label || 'Overview'
  return <div className={`app-shell theme-${theme}`}>
    <aside className={`sidebar ${open ? 'sidebar-open' : ''}`}>
      <div className="brand"><span className="brand-mark"><BusFront size={21} /></span><div><strong>Smart Bus</strong><span>Intelligence</span></div><button className="icon-btn mobile-close" onClick={() => setOpen(false)}><X size={18} /></button></div>
    ...
    <main className="main-shell">
      <header className="topbar"><button className="icon-btn menu-trigger" onClick={() => setOpen(true)}><Menu size={20} /></button><div className="breadcrumb"><span>City operations</span><ChevronRight size={14} /><strong>{pageLabel}</strong></div><div className="topbar-actions"><div className="topbar-live"><i /> Network live</div><button className="icon-btn" title={theme === 'dark' ? 'Switch to light mode' : 'Switch to dark mode'} onClick={onToggleTheme}>{theme === 'dark' ? <Sun size={18} /> : <Moon size={18} />}</button><button className="icon-btn" title="Notifications"><Bell size={18} />{notifications.length > 0 && <b>{notifications.length}</b>}</button><div className="avatar">AO</div></div></header>
