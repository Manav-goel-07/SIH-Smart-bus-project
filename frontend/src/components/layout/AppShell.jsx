...
export default function AppShell({ children, apiStatus, realtimeStatus, notifications = [], dismissNotification, theme = 'dark', onToggleTheme, profile }) {
  ...
  return <div className={`app-shell theme-${theme}`}>
    ...
    <main className="main-shell">
      <header className="topbar"><button className="icon-btn menu-trigger" onClick={() => setOpen(true)}><Menu size={20} /></button><div className="breadcrumb"><span>City operations</span><ChevronRight size={14} /><strong>{pageLabel}</strong></div><div className="topbar-actions"><div className="topbar-live"><i /> Network live</div><button className="icon-btn" title={theme === 'dark' ? 'Switch to light mode' : 'Switch to dark mode'} onClick={onToggleTheme}>{theme === 'dark' ? <Sun size={18} /> : <Moon size={18} />}</button><button className="icon-btn" title="Notifications"><Bell size={18} />{notifications.length > 0 && <b>{notifications.length}</b>}</button><ProfileMenu /><button className="icon-btn" title="Sign out" onClick={signOut}><LogOut size={16} /></button></div></header>
